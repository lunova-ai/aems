...
function generateSamples(
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    if (distribution === "normal") {
      ...
      const m =
        Math.log(mean) -
        0.5 * Math.log(1 + (stdDev * stdDev) / (mean * mean));
...
function percentile(sorted: number[], p: number): number {
  ...
  const lowerVal = sorted[lower] ?? sorted[0] ?? 0;
  const upperVal =
    sorted[upper] ?? sorted[sorted.length - 1] ?? sorted[0] ?? 0;
      ...
      <AEMSCard className="p-5 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <div className="text-xs text-gray-300 mt-1">
              {iterations.toLocaleString()} Läufe
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={meanCost}
              onChange={(e) =>
                setMeanCost(parseFloat(e.target.value))
              }
              className="w-full bg-white/5 border border-white/10 rounded-md px-2 py-1 text-xs text-gray-100"
            />
          </div>
        ...
        <div className="mt-4">
          ...
          <p className="mt-2 text-xs text-gray-400">
            Die Monte Carlo Simulation zeigt eine Bandbreite möglicher
            Energiekosten. P10–P90 helfen bei Budgetplanung,
            Risikobewertung und Vertragsstrategien.
          </p>
        </div>

        {hasRun && (
          <p className="text-xs text-aems-soft">
            Simulation durchgeführt – Ergebnis kann als Szenario im
            Executive Dashboard dokumentiert werden.
          </p>
        )}
      </AEMSCard>
    </AEMSSection>
